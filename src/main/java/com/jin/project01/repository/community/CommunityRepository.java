...
public interface CommunityRepository extends JpaRepository<Community, Integer> {
    ...
    // 특정 브랜드의 게시글 조회
    List<Community> findByCafeBrandMenuCafeBrandCafeBrandNoAndIsDeletedFalse(Integer cafeBrandNo);

    // 게시글 단건 조회 (삭제되지 않은 것)
    Optional<Community> findByCommunityNoAndIsDeletedFalse(Integer communityNo);

    List<Community> findByCafeBrandMenuCafeMenuNoAndIsDeletedFalse(Integer cafeMenuNo);
}
